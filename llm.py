    ...
    def generate(self, messages: list[dict]) -> str:
        if isinstance(self.llm, OpenAI):
            ...
            for attempt in range(max_retries):
                try:
                    response = self.llm.chat.completions.create(messages=messages, temperature=0.1, model=self.model)
                    break
            ...
            response = self.llm.create_chat_completion(messages=messages,temperature=0.1)
            return response["choices"][0]["message"]["content"]
